...
exports.init = function () {
    ...
    return method().then((responseSchema) => {
 ...
 */
function getSchema() {
    return fs.readJSON(paths.files.schema).catch((err) => {
        return Promise.reject(new Error("Couldn't read schema file: " + err.message));
    });
}
...
function fetchSchema() {
    return axios({
        ...
        .catch((err) => {
            return Promise.reject(new Error("Couldn't get schema from prices.tf API: " + err.message));
        });
}
